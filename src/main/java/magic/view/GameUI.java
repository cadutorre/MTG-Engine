...
import magic.effect.LoseLifeEffect;
...
public class GameUI extends JFrame implements GameStateObserver, PlayerController {
	...
	private static final long serialVersionUID = 1413115204056454185L;
	...
	public void effectExecuted(Effect<?> effect) {
		...
		if (effect instanceof EnterBattlefield) {
			...
			// TargetedEffect is sort of a wrapper and would just be noisy in
			// the log
			logEffect = false;
	...
	public <T> void chooseTarget(TargetedEffect<T> effect) {
		...
		T target = (T) JOptionPane.showInputDialog(this, effect.toString(), "Choose a target",
				JOptionPane.QUESTION_MESSAGE, null, targets, null);
	...
	public boolean promptOptionalEffect(Effect<?> effect) {
		return JOptionPane.showConfirmDialog(this,
				"Would you like to: " + effect.toString() + "?") == JOptionPane.OK_OPTION;
	}
	...
	public void declareBlockers(Player defendingPlayer, Combat combat) {
		...
		while (true) {
			...
			Creature attacker = (Creature) JOptionPane.showInputDialog(this, "Choose an Attacking Creature to Block",
					"Declare Blockers", JOptionPane.QUESTION_MESSAGE, null, attackers, null);
	...
	public Stackable offerPriority(Player player, boolean canPlaySorcery) {
		...
		// TODO to move the game along faster, use the three-second timer any
		// time they have no Sorceries in hand
		if (canPlaySorcery)
	...
	private Card modalForCard(List<Card> legalCardList) {
		...
		return (Card) JOptionPane.showInputDialog(this, prompt, "Play a Spell", JOptionPane.QUESTION_MESSAGE, null,
				legalCards, null);
	}
